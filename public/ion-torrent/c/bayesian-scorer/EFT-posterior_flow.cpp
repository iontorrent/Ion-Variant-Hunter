...
double flow_poster_prob_calc::best_hyp(flow_list *alter_hyp, flow_list *read_list, int cov, float vh)
{
    ...
    int *f;
    int len;
    if (num_r == 0 || num_hyp == 0) return 0.0;
    for (i = 0; i < num_hyp; i++) {
	int j;
	double prior;
...
static int add_read(FILE *fp, char *line, flow_list *rs, int adj) 
{
    ...
    int c = CONTEXT+1, fi;
    char d, dd;
    sscanf(line, "%*s %*s %*s %*s %c %*s %*s %*s %c:%d:%*c:%*d:%d", &d, &dd, &x, &fi);

    unsigned char d1 = code(dd);
    //printf("%c  %d %d %d %d %d\t", d, x, total_flow_order[x], fs[x], d1, fi);
	...
	if (t == 6) {fprintf(stderr, "Error can not find the flow: %sError %s", line, line2); return 1;}
	//printf("%c  %d %d %d %d %d\t", d, x, total_flow_order[x], fs[x], d1, fi);
    }
...
main(int argc, char *argv[])
{
    ...
    */
    ...
    if (argc < 4) {
	fprintf(stderr, "%s ref_file devfile flow_order_string output_vcffile [input_vcffile]", argv[0]);
    }
    ...
    int print_header = 1;
    if (argc > 5) {
	FILE *input_dev = ckopen(argv[5], "r");
	print_header = 0;
	while (fgets(line, sizeof line, input_dev)) {
	    if (line[0] == '#') fprintf(outp, line);
	    else break;
	}
    }
    int diff, coverage;
	...
	if (line[0] == '\n') break;
	if (line[0] == '#' && sizeof line > 1 && line[1] == '#') { 
	    if (print_header) fprintf(outp, line); 
	    continue;
	}
	if (out_header) {
	    out_header = 0;
	    // This is after all the ## in the vcf, but before header line, which has just one #
	...
	if (line[0] == '#') { // now print the single # header lines
	    if (print_header) fprintf(outp, line); 
	    continue;
	}
	char sid[100];
